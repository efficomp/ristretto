...
/**
 * 
 * Copyright (c) 2017, Jesús González
 */
 ...
 */
public class FSSensitivityValidator {
	...
	private static void readParams(String[] args) {
		...
		/*
		 * The forth argument is the label to be considered for the sensitivity
		 * analysis
		 */
		if (args.length < 4) {
		...
		/* Get the label for the sensitivity analysis */
		label = args[3];
	...
	private static void validate() {
		...
		double sensTraining = PerformanceIndexes.sensitivity(classifier, finalTrainingData, label);
		double sensTest = PerformanceIndexes.sensitivity(classifier, finalTestData, label);

		if (verbose) {
			System.out.printf("\nSensitivity Training: %.6f\n", sensTraining);
			System.out.printf("Sensitivity Test:     %.6f\n", sensTest);
		} else {
			System.out.printf("%.6f\t%.6f\n", sensTraining, sensTest);
		}
	}

	/**
	 * Obtain the training and test sensitivities of the results of a Feature
	 * Selector
	 * <p>
	 * <b>Expected args:</b>
	 * <ol>

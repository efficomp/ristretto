...
public class TestNormalization {
	...
	public static void main(String[] args) throws Exception {
		 ...
		 */
		Dataset data = FileHandler.loadDataset(new File(args[0]), Integer.parseInt(args[1]), "\\s+");
		MoreDatasetTools.normalize(data);
		...
		/*
		 * Load a data set for evaluation, this can be different, but we will use the
		 * same one.
		 */
		Dataset dataForClassification = FileHandler.loadDataset(new File(args[0]), Integer.parseInt(args[1]), "\\s+");
		MoreDatasetTools.normalize(dataForClassification);
